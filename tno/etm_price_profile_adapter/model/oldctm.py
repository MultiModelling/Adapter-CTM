...
# Now we have prepared the session IDs that we will work with
            ...
            if self.minio_client:
                self.minio_client.fput_object(self.model_run_dict[model_run_id].config.bucket_name, self.model_run_dict[model_run_id].config.output_file_name + 'post_ctm', 'files/' + self.model_run_dict[model_run_id].config.output_file_name)
